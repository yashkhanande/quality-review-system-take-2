...
export const createTemplate = asyncHandler(async (req, res) => {
  ...
  if (template) {
    ...
    console.log(
      `📝 POST /templates - Updated existing template with stages: ${stages.join(", ")}`,
    );
...
export const getTemplate = asyncHandler(async (req, res) => {
  ...
  console.log(
    `📋 GET /templates - Returning template with stages: ${actualStages.join(", ")}`,
  );
...
export const deleteChecklistFromTemplate = asyncHandler(async (req, res) => {
  ...
  console.log(
    `🗑️ DELETE checklist request - ID: ${checklistId}, Stage: ${stage}`,
  );
  ...
  if (!Array.isArray(template[stage])) {
    console.error(`❌ Stage ${stage} not found or not an array`);
    throw new ApiError(404, `Stage ${stage} not found or has no checklists`);
  }

  const before = template[stage].length;
  console.log(`📊 Before deletion: ${before} checklists in ${stage}`);
  ...
  if (!checklistExists) {
    console.error(`❌ Checklist ${checklistId} not found in ${stage}`);
    throw new ApiError(404, "Checklist not found in specified stage");
  }
  ...
  console.log(
    `✅ Checklist group deleted successfully (MongoDB modified count: ${updateResult.modifiedCount})`,
  );

  // Fetch updated template to return
  const updatedTemplate = await Template.findOne();
  const after = (updatedTemplate[stage] || []).length;
  console.log(`📊 After deletion: ${after} checklists remaining in ${stage}`);
...
export const deleteCheckpointFromSection = asyncHandler(async (req, res) => {
  ...
  console.log(
    `✅ Checkpoint deleted from section (MongoDB modified: ${updateResult.modifiedCount})`,
  );
...
export const deleteCheckpointFromTemplate = asyncHandler(async (req, res) => {
  ...
  console.log(
    `✅ Checkpoint deleted (MongoDB modified: ${updateResult.modifiedCount})`,
  );
...
export const deleteSectionFromChecklist = asyncHandler(async (req, res) => {
  ...
  console.log(
    `✅ Section deleted (MongoDB modified: ${updateResult.modifiedCount})`,
  );
...
export const addStageToTemplate = asyncHandler(async (req, res) => {
  ...
  console.log(
    `✅ Stage ${stage} added successfully${stageName ? ` with name "${stageName}"` : ""}`,
  );
  ...
  console.log(
    `✅ Verified in DB - Template now has stages: ${savedStagesInDb.join(", ")}`,
  );
...
export const deleteStageFromTemplate = asyncHandler(async (req, res) => {
  ...
  if (template[stage] === undefined) {
    console.warn(
      `⚠️ Stage ${stage} not found. Available: ${availableStages.join(", ")}`,
    );
    throw new ApiError(
      404,
  ...
  console.log(
    `✅ Stage ${stage} deleted successfully (modified: ${updateResult.modifiedCount})`,
  );
...
export const getAllStages = asyncHandler(async (req, res) => {
  ...
  console.log(
    `📋 Retrieved ${stageKeys.length} stages: ${stageKeys.join(", ")}`,
  );
...
export const resetTemplate = asyncHandler(async (req, res) => {
  ...
  console.log(
    `🗑️ Template reset - Deleted ${result.deletedCount} template document(s)`,
  );
