...
const compareAnswers = asyncHandler(async (req, res) => {
  ...
  const [exec, rev] = await Promise.all([
    ChecklistAnswer.find({ project_id: projectId, phase: phaseNum, role: 'executor' }),
    ChecklistAnswer.find({ project_id: projectId, phase: phaseNum, role: 'reviewer' }),
  ]);
  ...
  return res.status(200).json(new ApiResponse(200, { match, stats }, 'Comparison complete'));
});
...
const requestApproval = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, record, 'Approval requested'));
});
...
const approve = asyncHandler(async (req, res) => {
  ...
  const record = await ChecklistApproval.findOneAndUpdate(
    { project_id: projectId, phase: phaseNum },
    { $set: { status: 'approved', decided_at: new Date(), decided_by: req.user?._id || null } },
    { new: true, upsert: true }
  );
  ...
  const existing = await Stage.findOne({ project_id: projectId, stage_name: nextPhaseName });
  if (!existing) {
    await Stage.create({
  ...
  return res.status(200).json(new ApiResponse(200, record, 'Approved and advanced'));
});
...
const revert = asyncHandler(async (req, res) => {
  ...
  const record = await ChecklistApproval.findOneAndUpdate(
    { project_id: projectId, phase: phaseNum },
    { $set: { status: 'reverted', decided_at: new Date(), decided_by: req.user?._id || null, notes: notes || '' } },
    { new: true, upsert: true }
  );
  return res.status(200).json(new ApiResponse(200, record, 'Reverted to current stage'));
});
...
const getApprovalStatus = asyncHandler(async (req, res) => {
  ...
  const record = await ChecklistApproval.findOne({ project_id: projectId, phase: phaseNum });
  if (!record) throw new ApiError(404, 'Approval record not found');

  return res.status(200).json(new ApiResponse(200, record, 'Approval status fetched'));
});
